...
def get_reward(shapes, params, env, ep_max_step, continuous_a, seed_and_id=None,):    
    ...
    p = params_reshape(shapes, params)#将params变成矩阵
    # run episode
    s = env.reset()
    ep_r = 0.
...
def linear(n_in, n_out):  #线性全连接层
    w = np.random.randn(n_in * n_out).astype(np.float32) * .1
    b = np.random.randn(n_out).astype(np.float32) * .1
    return (n_in, n_out), np.concatenate((w, b))
   
def build_net(): #使用numpy搭建神经网络
...
def train(net_shapes, net_params, optimizer, utility, pool):
    ...
    noise_seed = np.random.randint(0, 2 ** 32 - 1, size=N_KID, dtype=np.uint32).repeat(2)    # 生成噪点的seed

    # 用多进程完成get_reward功能
    jobs = [pool.apply_async(get_reward, (
        net_shapes, net_params, env, CONFIG['ep_max_step'], 
