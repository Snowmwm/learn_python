...
train_dataset = Data.TensorDataset(data_tensor=torch.unsqueeze(train_set[0], dim=1), target_tensor=train_set[1])
#2、把dataset放入DataLoader
train_loader = Data.DataLoader(dataset=train_dataset, batch_size=128,shuffle=True)

test_x = Variable(torch.unsqueeze(test_set[0], dim=1), volatile=True).type(torch.FloatTensor)/255.0
test_y = test_set[1]
"""
